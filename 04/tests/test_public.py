...
class FrozenLakeWrapper():
    ...
    def reset(self):
        ...
        """        
        retval = self.gym_env.reset() #returns obs or obs, info
        if int(gym.__version__.split('.')[1]) > 25:
            return retval[0]
        else:
            return retval
    ...
    def step(self, action):
        ...
        self.num_steps += 1
        next_obs, reward, done, *_ = self.gym_env.step(action) #returns observation, reward, done, truncated, info OR observation, reward, done, info
...
def test_q_learning(desc, Q_mean, Q_std):
    ...
    env = FrozenLakeWrapper(
        gym.make(
            ...
            desc=desc,
            is_slippery=True
#           ,render_mode="human"  #to render in version 0.26.0 or higher add render mode here
        )
    )
    Q = q_learning(env)
